...
public class RobotSpeedGraphPanel extends JPanel {
    ...
    private Chart createOrderChart(RobotLog l, int order) {
        ...
        if (order == 0) {
            ...
            axis.addLabel(new AxisLabel(df.format(cLeft.getLowerYRange().doubleValue()), cLeft.getLowerYRange().doubleValue()));
            axis.addLabel(new AxisLabel(df.format(cLeft.getHigherYRange().doubleValue()), cLeft.getHigherYRange().doubleValue()));
            cLeft.setLeftAxis(axis);
        }
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                try {
                   ...
                   // downloadLog(file, true);
                    UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
                    Sketcher s = new Sketcher();
                    s.setVisible(true);
                    JFrame f = new JFrame();

                    f.setContentPane(new RobotSpeedGraphPanel(new RobotLog(file)));
            ...
            private void downloadLog(File file, boolean ev3) throws IOException, FileNotFoundException {

                Connection connection;
                if (ev3)
                    connection = new Connection("192.168.2.240");
                else
                    connection = new Connection("192.168.2.110");
                connection.connect();
                if (ev3)
                    connection.authenticateWithPassword("root", "");
                else
                    connection.authenticateWithPassword("maillard", "guigui");
                ...
                if (ev3) {
                    ...
                    cl.get("/home/maillard/EV3/workspace/EV3/LinuxX86/log.txt", fileOutputStream);
                }
                fileOutputStream.close();
                System.err.println("Log retrieved to " + file.getAbsolutePath());
                if (!file.exists()) {
                    System.err.println("Log retrieved to " + file.getAbsolutePath() + " MISSING");
                }
            }
        });
