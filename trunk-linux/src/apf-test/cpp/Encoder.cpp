...
:ss_port(slave_select_port_letter), ss_pin(slave_select_pin)
{
	int32_t ret;
	...
	gpio_dev = as_gpio_open(slave_select_port_letter, slave_select_pin);
	if (gpio_dev == NULL)
	{
		printf("Error can't open gpio %c\nHave you run loadgpio.sh ?\n", slave_select_port_letter);
		exit(1);
	}
	ret = as_gpio_set_pin_direction(gpio_dev, 1); //pin Direction (0:in, 1:out) => OUT
	if(ret < 0)
	{
	...
	long speed = as_spi_get_speed(spi_fd);
...
//  delayMicroseconds(10);        // Might be required for IC timing
  ...
  return SPDR;                    // return the received byte*/
	...
	unsigned long long result = as_spi_msg(spi_fd,
