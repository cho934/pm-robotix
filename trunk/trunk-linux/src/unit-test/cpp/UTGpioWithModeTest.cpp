...
#include <cstring>
#include "UTGpioWithModeTest.hpp"
...
std::string test::UTGpioWithModeTest::INPUT_PORT_PATH_DEV = "/dev/gpio/P"; // écriture (write) et lecture (read) blocante sur pin
std::string test::UTGpioWithModeTest::INPUT_PORT_PATH_PROC = "/proc/driver/gpio/port"; //lecture port/dir/irq : cat /proc/driver/gpio/portD

void test::UTGpioWithModeTest::run(int argc, char *argv[])
{
    ...
    std::cout << "APF : Test de different mode de -Read- et -Blocking Read- via GPIO." << std::endl;

    if (argc < 4)
    {
        std::cout << "USAGE: PMX_TEST " << argv[1] << " [GPIO_PORT:A B C D] [GPIO_PIN:0...31] [V1...4]" << std::endl;
    }
    if (argc > 2)
    {
        inGpioPort = argv[2];
 ...
 *
 */
void test::UTGpioWithModeTest::in_V4_ioctl_for_port(std::string, std::string)
{

    logger().info() << "V4 - compte les fronts (sans blocking-read) par ioctl " << utils::end;
        ...
        */
        ...
        if(i != lasti)//front montant et descendant
        {
            if(i > lasti)//front montant
            {
                ...
                logger().info() << " nbi: " << nbi << " ;nbtest: " << nbtest << " ;time-us:" << now << " ;diff-time-us:" << now - last
                    << " ;pin=" << i << " " << lasti << " ;DephG:" << dephG << " ;DephD:" << dephD << utils::end;
                last = now;
                nbi++;
                nbtest=0;
            }
...
void test::UTGpioWithModeTest::in_V3_ioctl_for_pin(std::string inGpioPort, std::string inPin)
{
    ...
    logger().info() << "V3 - compte les fronts (sans blocking-read) par ioctl " << utils::end;
    ...
    while (now < start + 3000000)
    {
        ...
        if(i != lasti)//front montant et descendant
        {
            if(i > lasti)//front montant
            {
                now = chrono.getElapsedTimeInMicroSec();
                logger().info() << " nbi: " << nbi << " ;nbtest: " << nbtest << " ;time-us:" << now << " ;diff-time-us:" << now - last
                    << " ;pin=" << i << " " << lasti << " ;DephG:" << valdephG << " ;DephD:" << valdephD << utils::end;
                last = now;
                nbi++;
                nbtest=0;
            }
 ...
 * 
 */
void test::UTGpioWithModeTest::in_V2_blockingread(std::string inGpioPort, std::string inPin)
{
    logger().info() << "V2 - Blocking-read with as_devices : as_gpio_blocking_get_pin_value() " << utils::end;
    int i = 0;
    utils::Chronometer chrono;
    chrono.start();
    long now = chrono.getElapsedTimeInMicroSec();
    ...
    while (now < start + 3000000) //sur 3 secondes, doit obtenir 710 imp, moteurs à fond
    {
        ...
        if (ret != -10) //si different de timeout
        {
            ...
            now = chrono.getElapsedTimeInMicroSec();
            logger().info() << "Read:" << i << " ;time-us:" << now << " ;diff-time-us:" << now - last
                    << " ;DephG:" << pinDephG << " ;DephD:" << pinDephD << utils::end;
            last = now;
        }
    }
    as_gpio_close(pd24);
 ...
 * 
 */
void test::UTGpioWithModeTest::in_V1_blockingReadFstream(std::string inGpioPort, std::string inPin)
{
    ...
    logger().info() << "V1 - Blocking-read read() + lecture fstream du portD:" << filePort << utils::end;
    ...
    while (now < start + 3000000)
    {
        ...
        io.close();
        now = chrono.getElapsedTimeInMicroSec();
        logger().info() << filePort << " : " << nbtest << " time: " << now - last << " " << iomask << utils::end;
