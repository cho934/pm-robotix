...
pmx::LedBar::LedBar(pmx::Robot & robot)
		: ARobotElement(robot), actionStopped_(false), actionRunning_(false), position_(0), status_(0), nb_(0), timeus_(
				50000), hex_(0), hexNext_(0)
{
}
...
bool pmx::LedBarAction::execute() //IRobotOut * out
{
	...
	{
	case pmx::LEDBARSET:
		pmx::LedIndicator::instance().set(ledBar_.position(), ledBar_.status());
	...
	case pmx::LEDBARFLASHVALUE:
		pmx::LedIndicator::instance().flashValue(ledBar_.hexValue());
	...
	case pmx::LEDBARBLINK:
		...
		{
			pmx::LedIndicator::instance().flashValue((255 * (i_ % 2)));
	...
	case pmx::LEDBARALTERNATE:
		...
		if (chrono_.getElapsedTimeInMicroSec() >= lastTime_ + (long) ledBar_.timeus() || i_ == 1)
		{
			...
			else
			{
				...
				pmx::LedIndicator::instance().flashValue(ledBar_.hexValueNext());
			}
			i_++;
			lastTime_ = chrono_.getElapsedTimeInMicroSec();
		}
	...
	case pmx::LEDBARK2MIL:
		...
		if (chrono_.getElapsedTimeInMicroSec() >= lastTime_ + (long) ledBar_.timeus() || i_ == 1)
		{
			...
			{
				pmx::LedIndicator::instance().setOn(k_);
				//out->setValue(ledBar_.dm(), k_, true);
			}
			//out->setValue(ledBar_.dm(), j_, false);
			pmx::LedIndicator::instance().setOff(j_);
...
void pmx::LedBar::startSet(ushort position, bool status)
{
...
void pmx::LedBar::startFlashValue(uint hexValue)
{
	this->actionStopped_ = false;
	this->hex_ = hexValue;
	//ajout de l'action de set
