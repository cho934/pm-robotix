...
int32 Accel) {
	...
	P1 = out_cmd->Acceleration * out_cmd->period1 * out_cmd->period1 / 2
			+ out_cmd->V0 * out_cmd->period1;
...
EXPORTED_FUNCTION void ComputeSpeedCommandTime(SpeedCommand *out_cmd,
int32 V0,
int32 Time,
int32 Accel) {
	out_cmd->order0 = 0;
...
EXPORTED_FUNCTION BOOL GetSpeedOrder(SpeedCommand *speedCmd, int32 PeriodN,
		int32 *out_order) {
	BOOL finished = FALSE;
	...
	case SP_ACCEL_PHASE:
		if (PeriodN >= speedCmd->period1) {
			...
			*out_order = speedCmd->Acceleration * Tn
					+ speedCmd->Acceleration / 2 + speedCmd->V0
					+ speedCmd->lastOrder;
			break;
		}
