...
void computeAlphaDelta(float x1, float y1, float theta1, float x2, float y2,
		float *out_angle, float *out_dist) {
	float dx, dy;
	float dTheta;
...
TRAJ_STATE motion_GoToSpeedPath(const Position *pos, int nb) {
	...
	while (i < nb) {
		if ((fabs(angle) >= 0.5f) || (dist >= 0.10f)) {
			...
			motion_SpeedControlADMaxTime(&cmd, spAlpha, 0.5f, spDelta, 0.5f,
					50);
			path_LaunchTrajectory(&cmd);

			//compute displacement needed from current position during waiting time
			pos_GetPositionXYTheta(&x1, &y1, &theta1);
			computeAlphaDelta(x1, y1, theta1, pos[i].x, pos[i].y, &angle,
					&dist);
...
//TSL = Turn - Stop - Line
TRAJ_STATE motion_GoToTSLFrom(float x1, float y1, float theta1, float x2,
		float y2) {
	TRAJ_STATE state;
