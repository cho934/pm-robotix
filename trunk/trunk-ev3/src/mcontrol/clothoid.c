...
float FresnelCSSqrt(float x)
{
	...
	else if(x < 0.8f)
	{
		...
		return(0.2710734E1-0.1064432E1*x+0.121025E1*t3-0.1761708E1*t3*t2+0.273796E1
		*t7-0.4446194E1*t7*t2);
	}
	else
	{
		//code generated with maple
		...
		return(0.1941357E1-0.2476891*x+0.3094215E-1*t3-0.115775E-1*t3*t2+
		0.4600835E-2*t7-0.1644186E-2*t7*t2);
	}
}
...
float FresnelSSqrt(float x)
{
	...
	t3 = t2*t2;
	t7 = t3*t3;
	return(-0.6174085E-1+0.1591549*x-0.1266515E-1*t3-0.4615735E-2*t3*t2+
	0.3905568E-3*t7+0.2106704E-4*t7*t2);
}
