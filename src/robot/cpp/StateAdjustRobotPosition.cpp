...
pmx::IAutomateState*
pmx::automate::StateAdjustRobotPosition::execute(Robot& robot)
{
    ...
    robot.base().pidSpeedEnabled(true);
    robot.base().vecteurCorrectionEnabled(false);
    robot.base().movexy(robot.position().x(), 200, pmx::BFORWARD);
    while (robot.base().arrived() == false)
    {
        usleep(10000);
    }
    ...
    if (robot.myColor() == pmx::RED)
    {
        ...
        robot.firePulseEnabled(false);
    }else if (robot.myColor() == pmx::BLUE)
    {
        //tourner à droite de 90°
        logger().info() << "turn right" << utils::end;
        robot.base().turnRight(M_PI_2);
...
*/
    ...
    if (robot.rebootContact().state())
    {
        ...
        result = new pmx::automate::StateWaitForStart(); //si appui sur rebootContact
    }else
    {
        result = new pmx::automate::StateInitialize(); //si changement de couleur
    }
