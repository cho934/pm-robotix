...
{
    logger().info() << "ConfigurationTest : Go" << utils::end;
    this->testLoad();
    this->testGetString();
    this->testGetDouble();
    this->testGetInt();
...
test::ConfigurationTest::testLoad()
{
    ...
    try
    {
        logger().info() << "load test-configuration-ok.txt" << utils::end;
