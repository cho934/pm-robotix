...
test::PointerListTest::testConstructorInitialized() {
    ...
    instance.push_front(&str);
    this->assert(instance.size() == 1, "Le constructeur initialisé ne fournit pas le nombre de résultats attendus");

}

void
test::PointerListTest::testConstructorCopy() {
    utils::PointerList<std::string*> list;
    std::string str = "hello";
    list.push_front(&str);
    utils::PointerList<std::string*> instance(list);
    this->assert(instance.size() == 1, "Le constructeur par copie ne fournit pas le nombre de résultats attendus");
}
...
test::PointerListTest::testDestructor() {
    utils::PointerList<std::string*> * instance1 = new utils::PointerList<std::string*>();
    try {
        delete instance1;
    }
