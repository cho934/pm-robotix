...
void test::RobotFindPidTest::run(int argc, char *argv[])
{
    ...
    if (argc < 5)
    {
        std::cout << "USAGE: PMX_TEST " << argv[1] << " [tickG=1...100] [tickD=1...100] [Tps(sec)] [PID_G] [PID_D] [1=FORWARD 2=BACK]" << std::endl;
    }

    if (argc > 2)
    {
        SpeedMotorLeft = atoi(argv[2]);
        std::cout << "Vitesse de départ moteur G en tick [1...100]: " << atoi(argv[2]) << std::endl;
    }else
    {
        //demande de la puissance moteur gauche à appliquer
        std::cout << "Vitesse de départ moteur G en tick [1...100]: " << std::flush;
    ...
    if (argc > 3)
    {
        ...
        std::cout << "Vitesse de départ moteur D en tick [1...100]: " << atoi(argv[3]) << std::endl;
    }else
    {
        //demande de la vitesse moteur droit à appliquer
        std::cout << "Vitesse de départ moteur D en tick [1...100]: " << std::flush;
    ...
    robot.base().vecteurCorrectionEnabled(false); //Pas de correction vectorielle
    ...
    robot.base().pidSpeedInitialize(pmx::MLEFT, leftP, leftI, leftD);
    logger().debug() << "pidCorrectionEnabled" << utils::end;
    robot.base().pidSpeedInitialize(pmx::MRIGHT,rightP, rightI, rightD);
    ...
    if (wayToGo == 2)
    {
        //activation du speedmanagerFusion
        logger().debug() << "pidCorrectionEnabled" << utils::end;
        robot.base().pidSpeedBackInitialize(pmx::MLEFT, leftP, leftI, leftD);
        robot.base().pidSpeedBackInitialize(pmx::MRIGHT,rightP, rightI, rightD);
    ...
    logger().info() << "Vitesse :" << SpeedMotorLeft << utils::end;
    logger().debug() << "Sleep " << startTime << utils::end;
    usleep(startTime * 1000000);

    //Changenent de vitesse
    logger().info() << "Changement de vitesse +30 :" << SpeedMotorLeft+30 << utils::end;
    robot.base().motorLeft().orderTick(SpeedMotorLeft+30);
    robot.base().motorRight().orderTick(SpeedMotorRight+30);
    usleep(startTime * 1000000);
/*
