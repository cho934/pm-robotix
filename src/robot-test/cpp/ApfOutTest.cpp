/*!
 * \file
 * \brief Implémentation de la classe ApfOutTest, Clignotement d'une sortie (ou pad) de la carte DevLightV2.
 */
...
void test::ApfOutTest::run(int argc, char *argv[])
{
    using std::string;
    char *ledGpioPort;
    char *filePort;
    ...
    if(argc < 3)
    {
        std::cout << "USAGE: PMX_TEST " << argv[1] <<" [GPIO_PORT:A B C D] [GPIO_PIN:0...31] [count:4 times (default)] [delay:100000us (default)]" << std::endl;
    }
    if(argc > 2)
    {
        ledGpioPort = argv[2];
    ...
    {
        ledPin = atoi(argv[3]);
        //std::cout << "LED_PIN: " << atoi(argv[3]) << std::endl;
    }
    else
    {
        //demande de la pin
    ...
    {
        count = atoi(argv[4]);
        //std::cout << "count: " << atoi(argv[4]) << " times" << std::endl;
    }
    if(argc > 5)
    {
        delay = atoi(argv[5]);
        //std::cout << "delay: " << atoi(argv[5]) << " times" << std::endl;
    }
