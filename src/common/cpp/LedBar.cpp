...
: ARobotElement(robot), dm_(dm), actionStopped_(false), position_(0), status_(0), nb_(0), timeus_(50000)
{}
...
bool pmx::LedBarAction::execute(IRobotOut * out)
{
    ...
    switch(this->action_)
    {
        ...
        case pmx::LEDBARBLINK :
            ...
            if(chrono_.getElapsedTimeInMicroSec() >= lastTime_ + ledBar_.timeus())
            {
...
void pmx::LedBar::k2Mil(int timeus)
{
...
void pmx::LedBar::blink(int nb, int timeus)
{
    this->nb_ = nb;
    this->timeus_ = timeus;
    //ajout de l'action de set
