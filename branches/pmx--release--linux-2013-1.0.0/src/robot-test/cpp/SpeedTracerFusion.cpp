...
test::SpeedTracerFusion::SpeedTracerFusion(pmx::Base * base) : base_(base),
        lastTimeUp_(0), nbPulsesLeft_(0), nbPulsesRight_(0),
        lastNbPulsesLeft_(0), lastNbPulsesRight_(0),
        timerStf_(PERIOD_TIMER, 0, this)
{
    ...
    printf(" Thread SpeedTracerFusion Start \n");
    chronoTime_.start();//Lancement du chrono de log
    timerStf_.start(); //Lancement du onTimer
}

void test::SpeedTracerFusion::onTimerEnd(utils::Chronometer)
{
...
void test::SpeedTracerFusion::onTimer(utils::Chronometer chrono)
{
    ...
    double distanceLeft = pmx::PositionManager::DISTANCE_PER_MOTOR_PULSE * (nbPulsesLeft - lastNbPulsesLeft_);
    ...
    //distance parcourue depuis le dernier calcul en mm
    double distanceRight = pmx::PositionManager::DISTANCE_PER_MOTOR_PULSE * (nbPulsesRight - lastNbPulsesRight_);
...
{
    double speed1 = 50; //en tick
    double speed2 = 100; //en tick
    ...
    if (motorCode1Left == -1)
    {
        logger.error() << "computeApproximation : MotorLEFT : erreur sur motorCode1" << utils::end;
        return;
    }
    
    if (motorCode2Left == -1)
    {
        logger.error() << "computeApproximation : MotorLEFT : erreur sur motorCode2" << utils::end;
        return;
    }
