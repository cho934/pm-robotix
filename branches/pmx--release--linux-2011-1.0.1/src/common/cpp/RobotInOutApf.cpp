...
void
pmx::RobotInOutApf::setValues(ushort element, ushort value)
{
    if (element == IRobotOut::DAC_MOTOR_LEFT)
    {
        logger().debug() << "setValues on DAC_MOTOR_LEFT [value:" << value << "]" << utils::end;
    ...
    }else if (element == IRobotOut::DAC_MOTOR_RIGHT)
    {
        logger().debug() << "setValues on DAC_MOTOR_RIGHT [value:" << value << "]" << utils::end;
...
pmx::RobotInOutApf::getValue(ushort element, ushort pin)
{
    if (element == IRobotIn::ADC_USE)
    {
        ...
        {
            logger().debug() << " pin:" << pin << " mV:" << value << utils::end;
            return value;
        }
    }else
    {
