...
utils::SvgAppender::SvgAppender(const std::string & filename) : ofs_(filename.c_str())
{
    ...
    this->ofs_ << "<svg width=\"" << (abs(xmin) + std::abs(xmax)) << "px\" height=\"" << (std::abs(ymin) + std::abs(ymax)) << "px\"" << std::endl;
    this->ofs_ << "xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">" << std::endl;
    ...
    this->ofs_ << "<defs>"
        ...
        << "<symbol id=\"robot-pmx\">"
        << "<circle cx=\"190\" cy=\"190\" r=\"190\" fill=\"none\" stroke=\"black\" />"
        << "<circle cx=\"190\" cy=\"190\" r=\"10\" fill=\"none\" stroke=\"black\" />"
    ...
    for (int i = ymax; i >= ymin; i -= 100)
    {
        ...
        this->ofs_ << "<text x=\"0\" y=\"" << i << "\" dx=\"5\" dy=\"-5\" fill=\"#444444\" font-size=\"12\">" << -i << "</text>" << std::endl;
    }
    ...
    for (int i = xmin; i <= xmax; i += 100)
    {
        ...
        this->ofs_ << "<text x=\"" << i << "\" y=\"0\" dx=\"5\" dy=\"17\" fill=\"#444444\" font-size=\"12\">" << i << "</text>" << std::endl;
    }
