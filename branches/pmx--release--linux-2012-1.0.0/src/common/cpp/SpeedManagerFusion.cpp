...
{
    //printf("SPM\n");
    if (this->ActiveSMF_ ) //si activation //& vecteurMode_==0 //& this->base_->modeVecteurCorrectionEnabled()==false
    {
        timeElapsed_++;
...
logger().debug() << "test11" << utils::end;
    ...
    if (err > 10)
    {
        ...
        this->base_->robot().ledBar().k2Mil(25000);
