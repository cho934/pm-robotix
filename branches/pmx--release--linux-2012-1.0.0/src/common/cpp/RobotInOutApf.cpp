...
pmx::RobotInOutApf::RobotInOutApf() : IRobotIn(), IRobotOut(), utils::Mutex(), fileDescriptorD(open(FILE_D, O_RDWR))
{
    ...
    if (this->max5821_dev == NULL)
    {
        ...
        pmx::LedIndicator::instance().setOff(5);
        exit(0);
    }else
    {
        //power up des DAC
...
*/
    ...
    pmx::LedIndicator::instance().setOn(6); //ok
    ...
    if (max1027_dev == NULL)
    {
        ...
        pmx::LedIndicator::instance().setOff(6);
...
void
pmx::RobotInOutApf::setValues(ushort element, int value)
{
    logger().info() << "setValues :" << value << utils::end;
      this->lock();
    if (element == IRobotOut::MD25_MOTOR_LEFT) //Value de -128=>127 (mode1)
    {
        utils::Md25::instance().ensureSetSpeed(value, MD25_SPEED1_REG);
        
    }else if (element == IRobotOut::MD25_MOTOR_RIGHT) //Value de -128=>127 (mode1)
    {
        utils::Md25::instance().ensureSetSpeed(value, MD25_SPEED2_REG);
