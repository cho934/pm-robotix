...
void test::RobotMoveTest::run(int argc, char *argv[])
{
    ...
    robot.base().motorRight().applyMotorCode(targetX);
    robot.base().motorLeft().applyMotorCode(targetY);
    usleep(10000000);
...
*/
    logger().info() << "Arrived D: " << targetX / pmx::PositionManager::DISTANCE_PER_MOTOR_PULSE << " mm " << targetY / pmx::PositionManager::DISTANCE_PER_MOTOR_PULSE  << " mm " << utils::end;
