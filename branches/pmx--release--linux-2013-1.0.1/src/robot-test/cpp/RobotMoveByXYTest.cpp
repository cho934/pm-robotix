...
void test::RobotMoveByXYTest::run(int argc, char* argv[])
{
    ...
    std::cout << "USAGE: PMX_TEST " << (int)argv[1] <<" [capActivation:0-1] [OpponentDetect:0-1]  [[coordX(mm)] [coordY(mm)] [angle(deg -179...+180)] [FORWARD-BACK:0-1] ...] " << std::endl;
    ...
    //init position and color
    robot.myColorIs(pmx::PMXRED);
    robot.position().x(0.0);
    robot.position().y(500.0,pmx::PMXRED);
    robot.position().angle(0.0,pmx::PMXRED);

    logger().info() << "position xya = 0, 500, 0" << utils::end;
