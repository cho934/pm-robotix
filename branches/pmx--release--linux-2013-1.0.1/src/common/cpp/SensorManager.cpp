...
long s_countA; // \todo A renommer
long s_countB; // \todo A renommer
...
pmx::SensorManager::executeWithNoBlockingRead() {
    ...
    pd29 = as_gpio_open('D', 29); //IMPDD (falling interrupt)
    pd26 = as_gpio_open('D', 26); //IMPDD (rising interrupt)
    pd30 = as_gpio_open('D', 30); //IMPGD (falling interrupt)
   ...
   or until the process calls `execve'.  */
    ...
    if (0 != rt_intr_create(&s_intr_left, "IrqLeft", 184, 0)) {
         ...
         */
    if (0 != rt_intr_create(&s_intr_R_deph_rising, "IrqRightDephRising", 186, 0)) {
        ...
        pmx::LedIndicator::instance().blinkPin(5, 3, 100000);
        if (s_task_error == 0) {
            pmx::LedIndicator::instance().setOn(5); //ok
        }else {
            pmx::LedIndicator::instance().setOff(5); //nok
        }
        ...
        while (!stop_) {
            if (firePulseEnabled_) {
                ...
                long s_countA_Temp = s_countA;
                long delta_s_countA = s_countA_Temp - s_countA_Old;
                //traitement des codeurs gauche 
                if (delta_s_countA != 0 || deltaLeftMd25 != 0) //lance un fire uniquement s'il y a eu une impulsion.
                {
                    fireLeftPulse(deltaLeftMd25, delta_s_countA, chrono_);
                }
                countLeftMd25Old = countLeftMd25;
                s_countA_Old = s_countA_Temp;
                ...
                long s_countB_Temp = s_countB;
                long delta_s_countB = s_countB_Temp - s_countB_Old;
                if (delta_s_countB != 0 || deltaRightMd25 != 0) {
                    fireRightPulse(deltaRightMd25, delta_s_countB, chrono_);
                }
                countRightMd25Old = countRightMd25;
                s_countB_Old = s_countB_Temp;
            }
            ...
            usleep(5000); //pour laisser la main si besoin
        } //fin while
    }
