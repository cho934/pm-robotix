...
EXPORTED_FUNCTION void ComputePositionCommand(PositionCommand *out_cmd,
...
//			dist, VMax, Accel, Decel);
	...
	if (fabs(
			out_cmd->order0
					+ out_cmd->Acceleration * out_cmd->T01 * out_cmd->T01 / 2.0f
					+ out_cmd->VMax * out_cmd->T23
					- out_cmd->Deceleration * out_cmd->T23 * out_cmd->T23
							/ 2.0f) > fabs(out_cmd->order3)) {
		//case 1 : maximum speed isn't reachable
		int32 order02b;
		out_cmd->T23 = round(
				sqrtf(
						(out_cmd->order3 - out_cmd->order0)
								/ ((out_cmd->Deceleration
										/ (2.0f * out_cmd->Acceleration) + 0.5f)
										* out_cmd->Deceleration)));
		out_cmd->VMax = out_cmd->T23 * out_cmd->Deceleration;

		order02b = out_cmd->order3
				- ((out_cmd->VMax * out_cmd->T23)
						- (out_cmd->T23 * out_cmd->T23 * out_cmd->Deceleration
								/ 2)) - out_cmd->order0;
		...
		order1 = out_cmd->Acceleration * out_cmd->T01 * out_cmd->T01 / 2
				+ out_cmd->order0;
		order2 = out_cmd->order3
				- (out_cmd->Deceleration * out_cmd->T23 * out_cmd->T23 / 2);
		...
		delta = out_cmd->VMax * out_cmd->VMax
				- 2 * out_cmd->Deceleration * order23b;

		if (delta > 0) {
			if (dist > 0.0f)
				out_cmd->T23 = abs(
						(-out_cmd->VMax + sqrtf(delta))
								/ out_cmd->Deceleration);
			else
				out_cmd->T23 = abs(
						(-out_cmd->VMax - sqrtf(delta))
...
//printf("GetPositionOrder TR_ACCEL_PHASE\n");
		...
		if (Tn >= posCommand->T01) {
			...
			*out_Order = posCommand->Acceleration * Tn
					+ posCommand->Acceleration / 2 + posCommand->lastOrder;
			break;
		}
...
//printf("GetPositionOrder TR_DECEL_PHASE\n");
		...
		if (Tn >= posCommand->T23) {
			...
			*out_Order = -posCommand->Deceleration * Tn
					- posCommand->Deceleration / 2 + posCommand->VMax
